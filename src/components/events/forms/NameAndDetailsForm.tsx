...
export const NameAndDetailsForm: React.FC<NameAndDetailsFormProps> = ({
  initialData,
  onChange,
}) => {
  ...
  const handleChange = <K extends keyof NameAndDetailsData>(
    field: K,
    value: NameAndDetailsData[K]
  ) => {
  ...
  const handleCheckboxChange = (
    field: keyof NameAndDetailsData,
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-muted-foreground mb-6">
        All information entered will be displayed on the public registration
        page for this event.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2 items-end">
        ...
        <div className="flex flex-col gap-2.5 pt-1 md:pt-0 self-center mt-3 md:mt-0">
          <div className="flex items-center space-x-2">
            <Checkbox
              id="listingVisible"
              checked={data.listingVisible}
              onCheckedChange={(checked) =>
                handleCheckboxChange('listingVisible', checked)
              }
            />
            <Label
              htmlFor="listingVisible"
          ...
          <div className="flex items-center space-x-2">
            <Checkbox
              id="listingFeatured"
              checked={data.listingFeatured}
              onCheckedChange={(checked) =>
                handleCheckboxChange('listingFeatured', checked)
              }
            />
            <Label
              htmlFor="listingFeatured"
              className="text-sm font-normal cursor-pointer"
            >
              Listing Featured
            </Label>
      ...
      {data.language === 'EN' && (
        <div className="space-y-1">
          ...
          <Input
            ...
            onChange={(e) => handleChange('eventNameEn', e.target.value)}
            className="h-9"
          />
        </div>
      )}
      {data.language === 'FR' && (
        <div className="space-y-1">
          ...
          <Input
            id="eventNameFr"
            value={data.eventNameFr}
            onChange={(e) => handleChange('eventNameFr', e.target.value)}
            className="h-9"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-1">
          ...
          <Input
            ...
            onChange={(e) => handleChange('eventWebsite', e.target.value)}
            className="h-9"
          />
        </div>
      </div>
